...
ALTER TABLE field_activity DROP CONSTRAINT IF EXISTS field_activity_day_key;
ALTER TABLE field_activity ADD  CONSTRAINT field_activity_day_key UNIQUE (source, rep_key, activity_date);
CREATE INDEX IF NOT EXISTS field_activity_date_idx    ON field_activity (activity_date DESC);
CREATE INDEX IF NOT EXISTS field_activity_profile_idx ON field_activity (profile_id);

ALTER TABLE field_activity ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS field_activity_read ON field_activity;
CREATE POLICY field_activity_read ON field_activity
  FOR SELECT TO authenticated USING (TRUE);
-- Admins import/correct by hand (CSV importer); the feed writes with the
-- service key, which bypasses RLS.
DROP POLICY IF EXISTS field_activity_write ON field_activity;
CREATE POLICY field_activity_write ON field_activity
  FOR ALL TO authenticated
  USING (my_role() = 'admin') WITH CHECK (my_role() = 'admin');

DROP TRIGGER IF EXISTS field_activity_touch ON field_activity;
CREATE TRIGGER field_activity_touch BEFORE UPDATE ON field_activity
  FOR EACH ROW EXECUTE FUNCTION touch_updated_at();

-- ── Per-knock events ────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS field_knocks (
  id          UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  source      TEXT NOT NULL DEFAULT 'repcard',
...
ALTER TABLE field_knocks DROP CONSTRAINT IF EXISTS field_knocks_source_external_key;
ALTER TABLE field_knocks ADD  CONSTRAINT field_knocks_source_external_key UNIQUE (source, external_id);
CREATE INDEX IF NOT EXISTS field_knocks_at_idx ON field_knocks (knock_at DESC);

ALTER TABLE field_knocks ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS field_knocks_read ON field_knocks;
CREATE POLICY field_knocks_read ON field_knocks
...
CREATE OR REPLACE FUNCTION field_knocks_rollup()
RETURNS TRIGGER LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE
  d DATE := (NEW.knock_at AT TIME ZONE 'America/Phoenix')::date;
BEGIN
  INSERT INTO field_activity (source, profile_id, rep_name, rep_email, activity_date,
...
DROP TRIGGER IF EXISTS field_knocks_rollup_trg ON field_knocks;
CREATE TRIGGER field_knocks_rollup_trg AFTER INSERT ON field_knocks
  FOR EACH ROW EXECUTE FUNCTION field_knocks_rollup();
