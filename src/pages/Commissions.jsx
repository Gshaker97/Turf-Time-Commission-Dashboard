...
export default function Commissions() {
    ...
    ;(async () => {
      const [{ data: d }, { data: p }] = await Promise.all([
        supabase.from('deals').select('*')
          .gte('sale_date', mo.start)
          .lt('sale_date', mo.end)
          .order('sale_date', { ascending: false }),
        supabase.from('payments').select('*')
          .eq('user_id', profile.id)
          .gte('pay_date', mo.start)
          .lt('pay_date', mo.end),
      ])
      setDeals(d || [])
      setPayments(p || [])
  ...
  const myDeals = useMemo(() =>
    (deals || []).filter(d =>
      d.setter_id === profile?.id ||
      d.closer_id === profile?.id ||
      d.manager_id === profile?.id ||
      d.director_id === profile?.id ||
      d.vp_id === profile?.id
    ), [deals, profile?.id])
    ...
    <div style={{ background: '#1a1a1a', color: '#fff', minHeight: '100%' }}>
      ...
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        <div>
          ...
          <p className="text-[13px] text-white/40 mt-0.5">{profile?.name}</p>
        </div>
        <select
          value={selected}
          onChange={e => setSelected(Number(e.target.value))}
          className="text-[13px] px-3 py-2 rounded-lg border border-white/10 bg-white/5 text-white"
        >
          {months.map((m, i) => (
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        {[
          ...
          <div key={c.label} style={{ background: '#1e1e1e', border: '1px solid #2a2a2a', borderRadius: 12, padding: '14px 16px' }}>
            <div className="text-[11px] uppercase tracking-wider text-white/30 font-semibold mb-2">{c.label}</div>
      ...
      <div style={{ background: '#1e1e1e', border: '1px solid #2a2a2a', borderRadius: 12, overflow: 'hidden' }}>
        <div className="px-4 py-3 border-b border-white/5">
          <span className="text-[12px] uppercase tracking-wider text-white/30 font-semibold">
            Deals — {mo?.label}
          </span>
        </div>
